...
@Component({
    selector: 'dictionary-employee-lock-reasons-logs',
    templateUrl: './dictionary-employee-lock-reasons-logs.component.html',
    styleUrls: ['./dictionary-employee-lock-reasons-logs.component.sass'],
    animations: [fade]
})
export class DictionaryEmployeeLockReasonsLogsComponent implements OnInit, OnDestroy {
    ...
    /**
     * DictionaryEmployeeLockReasons in the shape of MatTableDataSource.
     */
    logEmployeeLockReasons = new MatTableDataSource();
    ...
    constructor(private service: DictionaryEmployeeLockReasonsService,
    ...
    ngOnInit() {
        ...
        this.route.queryParams.pipe(takeUntil(componentDestroyed(this))).subscribe(params => {
            if (params.constructor === Object && Object.keys(params).length !== 0) {
                this.getEmployeeLockReasonsLogs(params);
                this.exportCriterias = params;
            } else this.getEmployeeLockReasonsLogs();
        });
    }
     ...
     */
    private getEmployeeLockReasonsLogs(criterias?: DictionaryEmployeeLockReasonsFetchCriterias) {
        this.isRequesting = true;

        this.service
            .getDictionaryEmployeeLockReasonsLogs(this.id, criterias)
            .pipe(takeUntil(componentDestroyed(this)))
            .subscribe(
                response => {
                    this.logEmployeeLockReasons = response.data.items;
                    this.logTotalCount = response.data.totalCount;
                },
                (error: Response) => (this.isRequesting = false),
                () => {
                    this.isRequesting = false;
                    this.logEmployeeLockReasons.paginator = this.paginator;
                    this.logEmployeeLockReasons.sort = this.sort;
                }
            );
    }
}
