...
@Component({
    selector: 'dictionary-nationalities-logs',
    templateUrl: './dictionary-nationalities-logs.component.html',
    styleUrls: ['./dictionary-nationalities-logs.component.sass'],
    animations: [fade]
})
export class DictionaryNationalitiesLogsComponent implements OnInit, OnDestroy {
     ...
     */
    logNationalities = new MatTableDataSource();
    ...
    ngOnInit() {
        ...
        this.route.queryParams.pipe(takeUntil(componentDestroyed(this))).subscribe(params => {
            if (params.constructor === Object && Object.keys(params).length !== 0) {
                this.getNationalitiesLogs(params);
                this.exportCriterias = params;
            } else this.getNationalitiesLogs();
        });
    }
     ...
     */
    private getNationalitiesLogs(criterias?: DictionaryNationalitiesFetchCriterias) {
        this.isRequesting = true;

        this.service
            .getDictionaryNationalitiesLogs(this.id, criterias)
            .pipe(takeUntil(componentDestroyed(this)))
            .subscribe(
                response => {
                    this.logNationalities = response.data.items;
                    this.logTotalCount = response.data.totalCount;
                },
                (error: Response) => (this.isRequesting = false),
                () => {
                    this.isRequesting = false;
                    this.logNationalities.paginator = this.paginator;
                    this.logNationalities.sort = this.sort;
                }
            );
    }
}
