...
@Component({
    selector: 'dictionary-user-lock-reasons-logs',
    templateUrl: './dictionary-user-lock-reasons-logs.component.html',
    styleUrls: ['./dictionary-user-lock-reasons-logs.component.sass'],
    animations: [fade]
})
export class DictionaryUserLockReasonsLogsComponent implements OnInit, OnDestroy {
    ...
    /**
     * DictionaryUserLockReasons in the shape of MatTableDataSource.
     */
    logUserLockReasons = new MatTableDataSource();
    ...
    ngOnInit() {
        ...
        this.route.queryParams.pipe(takeUntil(componentDestroyed(this))).subscribe(params => {
            if (params.constructor === Object && Object.keys(params).length !== 0) {
                this.getUserLockReasonsLogs(params);
                this.exportCriterias = params;
            } else this.getUserLockReasonsLogs();
        });
    }
     ...
     */
    private getUserLockReasonsLogs(criterias?: DictionaryUserLockReasonsFetchCriterias) {
        this.isRequesting = true;

        this.service
            .getDictionaryUserLockReasonsLogs(this.id, criterias)
            .pipe(takeUntil(componentDestroyed(this)))
            .subscribe(
                response => {
                    this.logUserLockReasons = response.data.items;
                    this.logTotalCount = response.data.totalCount;
                },
                (error: Response) => (this.isRequesting = false),
                () => {
                    this.isRequesting = false;
                    this.logUserLockReasons.paginator = this.paginator;
                    this.logUserLockReasons.sort = this.sort;
                }
            );
    }
}
