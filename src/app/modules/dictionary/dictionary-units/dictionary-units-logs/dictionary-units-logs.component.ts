...
@Component({
    selector: 'dictionary-units-logs',
    templateUrl: './dictionary-units-logs.component.html',
    styleUrls: ['./dictionary-units-logs.component.sass'],
    animations: [fade]
})
export class DictionaryUnitsLogsComponent implements OnInit, OnDestroy {
    ...
    /**
     * Units in the shape of MatTableDataSource.
     */
    logUnits = new MatTableDataSource();
    ...
    constructor(private service: DictionaryUnitsService,
    ...
    ngOnInit() {
        ...
        this.route.queryParams.pipe(takeUntil(componentDestroyed(this))).subscribe(params => {
            if (params.constructor === Object && Object.keys(params).length !== 0) {
                this.getUnitsLogs(params);
                this.exportCriterias = params;
            } else this.getUnitsLogs();
        });
    }
     ...
     */
    private getUnitsLogs(criterias?: DictionaryUnitsFetchCriterias) {
        this.isRequesting = true;

        this.service
            .getDictionaryUnitsLogs(this.id, criterias)
            .pipe(takeUntil(componentDestroyed(this)))
            .subscribe(
                response => {
                    this.logUnits = response.data.items;
                    this.logTotalCount = response.data.totalCount;
                },
                (error: Response) => (this.isRequesting = false),
                () => {
                    this.isRequesting = false;
                    this.logUnits.paginator = this.paginator;
                    this.logUnits.sort = this.sort;
                }
            );
    }
}
