...
@Component({
    selector: 'dictionary-useful-links-categories-logs',
    templateUrl: './dictionary-useful-links-categories-logs.component.html',
    styleUrls: ['./dictionary-useful-links-categories-logs.component.sass'],
    animations: [fade]
})
export class DictionaryUsefulLinksCategoriesLogsComponent implements OnInit, OnDestroy {
    ...
    /**
     * UsefulLinksCategories in the shape of MatTableDataSource.
     */
    logUsefulLinksCategories = new MatTableDataSource();
    ...
    ngOnInit() {
        ...
        this.route.queryParams.pipe(takeUntil(componentDestroyed(this))).subscribe(params => {
            if (params.constructor === Object && Object.keys(params).length !== 0) {
                this.getUsefulLinksCategoriesLogs(params);
                this.exportCriterias = params;
            } else this.getUsefulLinksCategoriesLogs();
        });
    }
     ...
     */
    private getUsefulLinksCategoriesLogs(criterias?: DictionaryUsefulLinksCategoriesFetchCriterias) {
        this.isRequesting = true;

        this.service
            .getDictionaryUsefulLinksCategoriesLogs(this.id, criterias)
            .pipe(takeUntil(componentDestroyed(this)))
            .subscribe(
                response => {
                    this.logUsefulLinksCategories = response.data.items;
                    this.logTotalCount = response.data.totalCount;
                },
                (error: Response) => (this.isRequesting = false),
                () => {
                    this.isRequesting = false;
                    this.logUsefulLinksCategories.paginator = this.paginator;
                    this.logUsefulLinksCategories.sort = this.sort;
                }
            );
    }
}
