...
@Component({
    selector: 'dictionary-files-categories-logs',
    templateUrl: './dictionary-files-categories-logs.component.html',
    styleUrls: ['./dictionary-files-categories-logs.component.sass'],
    animations: [fade]
})
export class DictionaryFilesCategoriesLogsComponent implements OnInit, OnDestroy {
    ...
    /**
     * FilesCategories in the shape of MatTableDataSource.
     */
    logFilesCategories = new MatTableDataSource();
    ...
    constructor(private service: DictionaryFileCategoriesService,
    ...
    ngOnInit() {
        ...
        // Set paginator values if filesCategory navigated from paginated link
        ...
        this.route.queryParams.pipe(takeUntil(componentDestroyed(this))).subscribe(params => {
            if (params.constructor === Object && Object.keys(params).length !== 0) {
                this.getFileCategoriesLogs(params);
                this.exportCriterias = params;
            } else this.getFileCategoriesLogs();
        });
    }
     ...
     */
    private getFileCategoriesLogs(criterias?: FileCategoriesFetchCriterias) {
        this.isRequesting = true;

        this.service
            .getFilesCategoriesLogs(this.id, criterias)
            .pipe(takeUntil(componentDestroyed(this)))
            .subscribe(
                response => {
                    this.logFilesCategories = response.data.items;
                    this.logTotalCount = response.data.totalCount;
                },
                (error: Response) => (this.isRequesting = false),
                () => {
                    this.isRequesting = false;
                    this.logFilesCategories.paginator = this.paginator;
                    this.logFilesCategories.sort = this.sort;
                }
            );
    }
}
