...
@Component({
    selector: 'dictionary-brands-logs',
    templateUrl: './dictionary-brands-logs.component.html',
    styleUrls: ['./dictionary-brands-logs.component.sass'],
    animations: [fade]
})
export class DictionaryBrandsLogsComponent implements OnInit, OnDestroy {
    ...
    /**
     * Brands in the shape of MatTableDataSource.
     */
    logBrands = new MatTableDataSource();
    ...
    constructor(private service: DictionaryBrandsService,
    ...
    ngOnInit() {
        ...
        this.route.queryParams.pipe(takeUntil(componentDestroyed(this))).subscribe(params => {
            if (params.constructor === Object && Object.keys(params).length !== 0) {
                this.getBrandsLogs(params);
                this.exportCriterias = params;
            } else this.getBrandsLogs();
        });
    }
    ...
    /**
     * Send search criterias to DictionaryBrandsService and get dictionary-brands
     * list in return
     * @param criterias Fetch criterias for DB searching
     */
    private getBrandsLogs(criterias?: DictionaryBrandsFetchCriterias) {
        this.isRequesting = true;

        this.service
            .getDictionaryBrandsLogs(this.id, criterias)
            .pipe(takeUntil(componentDestroyed(this)))
            .subscribe(
                response => {
                    this.logBrands = response.data.items;
                    this.logTotalCount = response.data.totalCount;
                },
                (error: Response) => (this.isRequesting = false),
                () => {
                    this.isRequesting = false;
                    this.logBrands.paginator = this.paginator;
                    this.logBrands.sort = this.sort;
                }
            );
    }
}
